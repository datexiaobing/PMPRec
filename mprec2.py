...
class Able2(nn.Module):
    def __init__(self, model_params):
        ...
        self.liner11 = nn.Linear(model_params['EDGE_EMBEDDING_DIM'],256)
        self.liner12 = nn.Linear(256,128)
        self.liner13 = nn.Linear(128,64)
        self.liner14 = nn.Linear(64,32)
        self.liner15 = nn.Linear(32, 1)
        # self.liner_cross = nn.Linear(32, 1)
        nn.init.xavier_uniform_(self.liner12.weight)
        nn.init.xavier_uniform_(self.liner11.weight)
        nn.init.xavier_uniform_(self.liner13.weight)
        nn.init.xavier_uniform_(self.liner14.weight)
        nn.init.xavier_uniform_(self.liner15.weight)
    ...
    def calculate(self, x):
        if self.node_att == "self_att":
            ...
            # print('h',h_node_id)
            h = self.embedding_all_node(h_node_id)
            # print('he',h.shape)

            t_node_id = x[:, -1:]
            t_node_id = torch.squeeze(t_node_id, dim=1)
            # print('t:', t_node_id)
            t = self.embedding_all_node(t_node_id)
            # print('te',t.shape)
            h_t_e = h * t
            # print(h_t_e.shape)
            # reshape node id to get node type id
        ...
        #     # print(pr)
        # else:
        pr = self.liner12(self.relu(pr))
        pr = self.liner13(self.relu(pr))
        pr = self.liner14(self.relu(pr))
        pr = self.liner15(self.leaky_relu(pr))
