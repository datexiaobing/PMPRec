...
from able import Model
from mprec2 import  Able2
...
if __name__ == '__main__':
    ...
    pre_fix= "/home/zwb/zwb/yelp/data/new/"
    # pre_fix = "/mnt/driver0/zwb/yelp/data/new/"
    # pre_fix = "/home/public/Public/hin_zwb/able/douban/data/new/"
    ...
    for node_att in ['att_bi']:
        # ,'self_att','able','att'
        print('start train mode :',node_att)
        model_params['node_att'] = node_att
        train_count = 4
        for train_file in ['train/2/'+str(train_count)]:
            train_rate = '_2_'+str(train_count)
            rats = [2]
            temp_model = 'temp_yelp' + train_rate + '.pth'
            # ,'train_80_1'
            ...
            for t in range(epochs):
                ...
                for file in files:
                    ...
                    for _, (X, y) in enumerate(train_dataloader):
                        ...
                        label_1 = torch.ones((X.shape[0], 1), dtype=torch.float)
                        label_0 = torch.zeros((X.shape[0], 1), dtype=torch.float)
                ...
                for rat in rats:
                    ...
                    model2 = Able2(model_params)

                    # print('loading paramas ... ', temp_model)
                    static_dic = torch.load(temp_model)
                    model2.load_state_dict(static_dic,strict=False)
                    ...
                    # 加载数据
                    train_rate_file = pre_fix + '/rate/'+str(train_count)+'/' + str(rat) + '/new_u_b.txt.' + str(rat)
                    test_rate_file = pre_fix + '/rate/'+str(train_count)+'/' + str(rat) + '/test_new_u_b.txt.' + str(rat)
